...
use anchor_lang::solana_program::{program_option::COption, system_instruction, system_program};
...
use crate::{
    error::ErrorCode,
    state::{
        frame_access::FrameReader,
        structured_cpi_patch::StructuredCpiPatch,
        structured_cpi_payload::{
...
fn resolve_pubkey_value(frame: &impl FrameReader, value: &PubkeyValue) -> Result<Pubkey> {
    match value {
...
fn require_system_program(program_id: &Pubkey) -> Result<()> {
    require!(
        ...
        ErrorCode::InvalidStructuredCpiProgram
    );
    Ok(())
}
...
fn read_typed(frame: &impl FrameReader, index: u8, expected: ValueType) -> Result<ValueBytes> {
    let ty = frame.read_value_type(index)?;
    require!(ty == expected, ErrorCode::LoadTypeMismatch);
    frame.read_bytes(index, expected)
}

fn read_u8(frame: &impl FrameReader, source: &Value) -> Result<u8> {
    Ok(read_typed(frame, source.index, ValueType::U8)?.as_slice()[0])
}

fn read_u16(frame: &impl FrameReader, source: &Value) -> Result<u16> {
    let bytes = read_typed(frame, source.index, ValueType::U16)?;
    Ok(u16::from_le_bytes(bytes.as_slice().try_into().unwrap()))
}

fn read_u64(frame: &impl FrameReader, source: &Value) -> Result<u64> {
    let bytes = read_typed(frame, source.index, ValueType::U64)?;
    Ok(u64::from_le_bytes(bytes.as_slice().try_into().unwrap()))
}

fn read_pubkey(frame: &impl FrameReader, source: &Value) -> Result<Pubkey> {
...
mod tests {
    use super::*;
    use crate::state::StructuredCpiPatch;
    ...
    impl FrameReader for MockFrame {
        fn index_count(&self) -> Result<u16> {
            Ok(self.types.len() as u16)
        }
        ...
        fn read_value_type(&self, index: u8) -> Result<ValueType> {
            Ok(self.types[index as usize])
        }

        fn read_bytes(&self, index: u8, ty: ValueType) -> Result<ValueBytes> {
            require!(self.types[index as usize] == ty, ErrorCode::LoadTypeMismatch);
            Ok(self.payloads[index as usize])
        }
    }
    ...
    fn token_patch_rejects_wrong_program_id() {
        ...
        assert_eq!(err, ErrorCode::InvalidStructuredCpiProgram.into());
    }
