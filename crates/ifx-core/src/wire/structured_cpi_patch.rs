...
pub enum StructuredCpiPatch {
    ...
    // ── Stake Program ─────────────────────────────────────────────────────
    /// Stake `Withdraw` (4): dynamic **lamports** (`u64` after 4-byte variant).
    StakeWithdraw { lamports: Value },
    /// Stake `Split` (3): dynamic **lamports** (`u64` after 4-byte variant).
    StakeSplit { lamports: Value },
    /// Stake `Deactivate` (5): no dynamic fields.
    StakeDeactivate,
...
impl StructuredCpiPatch {
    ...
    pub fn log_label(&self) -> &'static str {
        match self {
            ...
            Self::StakeWithdraw { .. } => "stake:withdraw",
    ...
    pub fn append_log_bindings(&self, sink: &mut impl PatchLogSink) -> bool {
        match self {
            ...
            Self::StakeWithdraw { lamports } | Self::StakeSplit { lamports } => {
                sink.patch_binding("lamports", *lamports)
            }
            Self::StakeDeactivate | Self::StakeDelegateStake => false,
        }
    }
}
