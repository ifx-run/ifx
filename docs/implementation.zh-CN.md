...
| 指令 | 作用 |
|------|------|
| `ifx_create_frame` | 创建 Frame PDA；分配 `tape` + `payload_at`；`cursor = 0`，`index_count = 0` |
| `ifx_reset_frame` | 重置草稿：`cursor = 0`，`index_count = 0`（lazy — 不清 tape；见 [frame-cu-optimization.zh-CN.md](./frame-cu-optimization.zh-CN.md)） |
| `ifx_close_frame` | 关闭 Frame，回收 rent |
| `ifx_let` | 按 `bindings` 顺序求值并 **append** 到 `tape` |
| `ifx_assert` | 求值 `cond: Expr` 为 bool |
| `ifx_patched_cpi` | CPI；从 Frame tape patch 模板 `data`（`Cpi` + `CpiPatch`；`patches` 必须非空） |
| `ifx_if_else` | 条件分支：每 arm 为 `Skip` / `Revert` / CPI 序列 | 对 `cond` 求值；执行 `then_arm` 或 `else_arm`（`IfElseArm`） |

- `ifx_let` 必须在 **transaction 顶层**（`stack height == 1`）。
...
- **账户空间:** `1 + 32 + 4 + 2 + 2 + 4 + (index_cap×2) + 4 + tape_len`
- **指令 discriminator:** 各 1 字节（`0`…`6`，见 `programs/ifx/src/constants.rs`）

### Reset 与 append

1. `ifx_reset_frame` 或 `ifx_create_frame`：`cursor = 0`，`index_count = 0`（`ifx_create_frame` 仍初始化空 tape；`reset` 不逐字节清零 tape / `payload_at`）。
...
[`LetBinding`](./typed-let-bindings.zh-CN.md) 为 **单一 wire enum**（tag `0`–`23`）。链上 binding **按序 append**；wire 上无 per-binding 字节 offset 字段。
...
| `9`–`23` | SPL Token / Token-2022 typed | 官方 unpack |
...
链下 `FrameScratch` 分配顺序 index；**类型由 `LetBinding` 变体决定**（`Eval` 由推断得出），不编入 `Expr::Value` / `CpiPatch` wire。读时经 `payload_at[index]` 定位 tape 字节。

---

## 5. 表达式（`Expr`）

扁平 enum：每个运算符一个 Borsh tag（`0`–`42`）。详见英文 [implementation.md](./implementation.md) §5。
...
- `ifx_patched_cpi(arm: Cpi)` — `patches` 必须非空
- `ifx_if_else { cond, then_arm, else_arm }`
- **`IfElseArm` wire（每 arm 一个 u8 tag）：** `0x00` skip · `0x01..=0xfe` = N 个 **`Cpi`** 步（N = tag）· `0xff` revert。每 arm 顺序执行 **1–254** 步；每步 **`PatchList`** = `U16LenVec`（空 = 静态，非空 = patched）。
- **`Cpi`：** `remaining[start..start+len]` = `[program, …cpi_accounts]`；模板 `data` + 可选 `patches`；invoke 前经 `payload_at[source.index]` 从 `Frame::tape` 拷贝到模板 `data`（当 patches 非空）
