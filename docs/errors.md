...
| 6002 | `UnauthorizedClose` | Only the frame close authority may close | Wrong signer on `ifx_close_frame` |
| 6003 | `InvalidCloseAuthority` | Invalid close authority | `Pubkey::default()` passed to `ifx_create_frame` |
...
| 6022 | `IndexCapReached` | Frame binding index cap reached | `index_count == index_cap` at append (binding slots exhausted) |
