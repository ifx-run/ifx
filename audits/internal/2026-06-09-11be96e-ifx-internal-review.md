English | [中文](./2026-06-09-11be96e-ifx-internal-review.zh-CN.md)
...
| **Commit (full)** | [`11be96eed3724291bd514ac659b8e4eb1f3ad0dd`](https://github.com/ifx-run/ifx/commit/11be96eed3724291bd514ac659b8e4eb1f3ad0dd) |
| **Commit (short)** | `11be96e` |
...
| **Phase 0 log** | `audits/scratch/11be96e/phase0.log` |
| **Prior review** | [2026-06-08-09a9114-ifx-internal-review.md](./2026-06-08-09a9114-ifx-internal-review.md) |
...
**Verdict:** No ❌ rows. Phase 0 (`security:preflight`, `npm test` **137 passing**, `cargo test` **49 passing**, `cargo audit` exit 0) passed on commit `11be96e`.

**Delta vs `09a9114`:** Re-review after squashed merge to `main`. Material additions: flat **`StructuredCpiPatch`** (29 wire tags) with program-id whitelist per family (`structured_cpi.rs:403-420`); **`Frame.authority`** write ACL (`frame_authority.rs`); **`Frame.generation`** (+8 B layout, lazy reset); **`Pubkey`** on tape + let tags 25–28; **`invoke_cpi`** no read-lock during nested CPI (`patched_cpi.rs:14-18`). Attacker pass: 0 `CONFIRM-RISK`. Test-gap closed for **`InvalidStructuredCpiProgram`** (6030) — `tests/ifx_negative.ts` + `structured_cpi.rs` unit tests.

Scratch (maintainer): `audits/scratch/11be96e/phase0.log`; prior reader/attacker merge at `audits/scratch/ef20418/` (same checklist rows; pre-squash tree).
...
| IFX-SEC-C03 | ⚠️ | Static/Raw: builder picks program id; **Structured** validates System/SPL id — `structured_cpi.rs:403-420` |
...
| IFX-SEC-F05 | ✅ | Static / RawPatched / **Structured** via `invoke_cpi`; `tests/ifx.ts`, `ifx_structured_cpi_initialize_mint.ts`, `ifx_negative.ts` InvalidStructuredCpiProgram (6030) |
...
| IFX-SEC-I02 | ✅ | `ifx.ts`, `ifx_negative.ts`, `ifx_let_coverage.ts`, `ifx_cpi_edges.ts`, `ifx_structured_cpi_initialize_mint.ts`, `ifx_frame_generation.ts`, `ifx_pubkey.ts`, `frame_cu_benchmark.ts`, … |
| IFX-SEC-I03 | ✅ | `cargo test` **49** passed (Phase 0) |
| IFX-SEC-I04 | ✅ | `security:preflight` in Phase 0 |
| IFX-SEC-I05 | ✅ | `npm test` **137** passing (Phase 0) |
