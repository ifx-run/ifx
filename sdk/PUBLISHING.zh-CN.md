...
# 或锁定：npm install @ifx-run/sdk@0.1.3
```

`latest` 指向 **`0.1.3`**（默认主网）。与 **`ifx-sdk@0.1.3`**、**`go-sdk@v0.1.3`** 使用同一 git revision。
...
不再维护单独的 `devnet` dist-tag — 集成方请使用 `npm install @ifx-run/sdk`（`latest` → `0.1.3`）。
